...
impl SowApp {
    pub(super) fn render_frame_ui_and_present(&mut self, sf: f32, frame: blade_graphics::Frame) {
        ...
        if self.input.dragging { self.input.camera_focus_target = None; }
        if let Some((world_x, world_y)) = self.input.camera_focus_target {
            ...
            self.clamp_camera_to_map();
            if (target_x - self.input.camera_x).abs() < 0.5 && (target_y - self.input.camera_y).abs() < 0.5 { self.input.camera_focus_target = None; }
        } else {
        ...
        self.ui.app.main_menu_state.wait_timer_secs = (self.ui.app.main_menu_state.wait_timer_secs - dt).max(0.0);
        if let Some(timer) = &mut self.ui.app.hud_state.spawn_timer_secs { *timer = (*timer - dt).max(0.0); }
        if let Some(sync) = &mut self.ui.app.hud_state.sync_state { sync.time_remaining = (sync.time_remaining - dt).max(0.0); }
        if self.ui.app.phase == ClientPhase::Playing { self.sync_hud_player_state(); }
        ...
        if !self.web_loader_hidden && self.ui.app.phase != ClientPhase::Splash { crate::loader::hide_web_loader(); self.web_loader_hidden = true; }
