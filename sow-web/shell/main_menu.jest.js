// Jest-only menu surface. It overrides the shared shell's account and store
// views so the Jest artifact stays anonymous, non-commercial, and
// external-link safe. Registration goes through the Jest platform popup
// (window.SOW_jestLogin); there is no first-party auth UI here.
...
// Local copy: the shared shell's localeOptions is stripped from portal
// bundles, so this file must not call it.
function jestLocaleOptions() {
...
function renderSettings() {
    ...
    var accountControl = "<div class='sow-menu__settings-account'><strong class='sow-menu__account-value'>" + esc(SOW_t("menu.anonymous")) + "</strong>" +
        "<button class='sow-menu__secondary' type='button' data-command='jest_login'>" + esc(SOW_t("menu.sign_in")) + "</button></div>";
    return "" +
...
document.addEventListener("click", function (event) {
    var target = event.target && event.target.closest ? event.target.closest("[data-command='jest_login']") : null;
    if (!target) return;
    event.preventDefault();
    if (typeof window.SOW_jestLogin === "function") {
        window.SOW_jestLogin();
    }
});
